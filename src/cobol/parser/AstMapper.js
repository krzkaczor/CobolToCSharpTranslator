...
module.exports = class AstMapper extends CobolVisitor {
    ...
    visitPicture(ctx) {
        //get raw PIC string
        var picString = _.tail(ctx.children).map(c => c.getText()).join('');

        return new nodes.Picture(picString);
    }
    ...
    visitIfThenEndIfStat(ctx) {
        return new nodes.IfStatement(this.visit(ctx.booleanExpression()), this.visit(ctx.statements()));
    }
